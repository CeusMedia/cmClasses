 ...
 *	@version		0.6
 */
class Framework_Xenon_Core_PageController
{
	...
	{
		import( 'de.ceus-media.framework.xenon.core.PageDefinitionEditor' );
		$editor	= new Framework_Xenon_Core_PageDefinitionEditor( $this->fileName );
	 ...
	 *	@return		bool
	 */
	public function getModule( $pageId )
	{
		if( !$this->checkPage( $pageId ) )
			throw new InvalidArgumentException( 'Page "'.$pageId.'" is not defined.' );
		$pageId	= strtolower( $pageId );
		$scope	= $this->getPageScope( $pageId );
		$page	= $this->pages[$scope][$pageId];
		return isset( $page['module'] ) ? $page['module'] : NULL;
	}
	...
	protected function readPages()
	{
		...
		{
			import( 'de.ceus-media.framework.xenon.core.PageDefinitionReader' );
			$reader			= new Framework_Xenon_Core_PageDefinitionReader( $this->getDocument() );
