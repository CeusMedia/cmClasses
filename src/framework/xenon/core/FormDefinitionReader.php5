 ...
 *	@version		0.6
 */
class Framework_Xenon_Core_FormDefinitionReader
{
	 ...
	 *	@return		void
	 */
	public function loadModuleDefinition( $module, $fileName, $force = false )
	{
		$prefix		= $this->prefix;
		$path		= $this->path;
		$fileName	= "module/".$module."/forms/".$fileName;
	 ...
	 *	@return		void
	 */
	public function loadDefinition( $fileName, $module = NULL, $force = false )
	{
		$prefix		= $this->prefix;
		$path		= $this->path;
		if( $module )
			$fileName	= "module/".$module."/forms/".$fileName;
		$xmlFile	= $prefix.$fileName.".xml";
		if( !$force && $this->useCache )
		{
			$cacheFile	= $this->getCacheFilename( $fileName );
			if( file_exists( $cacheFile ) && filemtime( $xmlFile ) <= filemtime( $cacheFile ) )
